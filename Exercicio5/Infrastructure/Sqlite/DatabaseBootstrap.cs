...
namespace Questao5.Infrastructure.Sqlite
{
    public class DatabaseBootstrap : IDatabaseBootstrap
    {
        ...
        public List<Conta> buscaDados()
        {
            using var connection = new SqliteConnection(databaseConfig.Name);

            var consulta = connection.Query<Conta>("select * from contacorrente").ToList();

            return consulta;

        }

        public Conta buscaDadosPorID(string id)
        {
            using var connection = new SqliteConnection(databaseConfig.Name);

            var consulta = connection.Query<Conta>(@$"select * from contacorrente where idcontacorrente = '{id}'").FirstOrDefault();

            return consulta;

        }

        public Conta buscaDadosPorConta(int id)
        {
            using var connection = new SqliteConnection(databaseConfig.Name);

            var consulta = connection.Query<Conta>(@$"select * from contacorrente where numero = '{id}'").FirstOrDefault();

            return consulta;

        }

        public Movimento InsereMovimento(Movimento movimento)
        {
            using var connection = new SqliteConnection(databaseConfig.Name);

            var consulta = connection.ExecuteScalar<Movimento>(@$"insert into movimento values
                                ('{movimento.idmovimento}', '{movimento.idcontacorrente}',
                                    '{DateTime.Now}','{movimento.tipomovimento}',{movimento.valor}) ");
            return consulta;
        ...
        {
            using var connection = new SqliteConnection(databaseConfig.Name);
            var movimentos = connection.Query<Movimento>(@$"select * from movimento m
                                                                            inner join  contacorrente c
                                                                            on 'm.idcontacorrente' = 'c.idcontacorrente'
                                                                            where c.numero = {numerodaConta}");
            double saldo = 0;
